...
<p align="center">特殊声明，本系统采用双端制，机器人不存储任何敏感数据，API私钥存储到你客户端服务器的本地，由bot驱动你的客户端服务器来操作，你可以随时关闭服务</P>
...
4.你的客户端服务器需未占用并且放行9527（或全部）端口（可以用telnet ip 端口测试），然后使用bash sh_client_bot.sh的启动客户端服务器
```
